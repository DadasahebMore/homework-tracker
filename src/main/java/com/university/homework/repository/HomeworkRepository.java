...
public interface HomeworkRepository extends JpaRepository<Homework, String> {
  ...
  List<Homework> findByAuthorIdAndStatusOrderByCreatedAtDesc(
      String authorId, Homework.HomeworkStatus status);

  List<Homework> findByVisibilityAndStatusOrderByCreatedAtDesc(
      Homework.Visibility visibility, Homework.HomeworkStatus status);

  @Query(
      "SELECT h FROM Homework h WHERE h.createdAt BETWEEN :from AND :to ORDER BY h.createdAt DESC")
  List<Homework> findByDateRange(@Param("from") LocalDateTime from, @Param("to") LocalDateTime to);

  @Query(
      "SELECT h FROM Homework h WHERE h.visibility = 'PUBLIC' AND h.status = 'PUBLISHED' "
