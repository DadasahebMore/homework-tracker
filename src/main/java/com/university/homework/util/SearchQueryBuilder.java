...
public class SearchQueryBuilder {
  ...
  public SearchSourceBuilder buildSearchSource(SearchRequestDTO request) {
    ...
    sourceBuilder.aggregation(
        AggregationBuilders.terms("tags_facet").field("tags.keyword").size(50));
    sourceBuilder.aggregation(
        AggregationBuilders.terms("authors_facet").field("authorName.keyword").size(20));
  ...
  private BoolQueryBuilder buildBoolQuery(SearchRequestDTO request) {
    ...
    if (request.getTitle() != null && !request.getTitle().isEmpty()) {
      boolQuery.must(
          QueryBuilders.matchQuery("title", request.getTitle()).boost(2.0f).fuzziness("AUTO"));
    }
