...
public class SearchService {
    ...
    @Cacheable(value = "homework-search", key = "T(java.lang.String).format('%s_%s_%d', " +
            "@com.university.homework.util.SearchQueryBuilder@generateCacheKey(#request), " +
            "#request.page, #request.pageSize)", unless = "#result.total == 0")
    public SearchResponseDTO search(SearchRequestDTO request) {
        ...
        try {
            ...
            log.info("Search completed in {} ms, found {} results",
                    System.currentTimeMillis() - startTime, result.getTotal());
    ...
    private SearchResponseDTO processSearchResponse(SearchResponse response,
                                                    SearchRequestDTO request,
                                                    long startTime) {
        SearchResponseDTO.SearchResponseDTOBuilder builder = SearchResponseDTO.builder()
                .success(true)
        ...
        long totalPages = (response.getHits().getTotalHits().value + request.getPageSize() - 1)
                / request.getPageSize();
        builder.totalPages((int) totalPages);

        // Convert hits to DTOs
        List<SearchResponseDTO.HomeworkResultDTO> results = Arrays.stream(response.getHits().getHits())
    ...
    private SearchResponseDTO.HomeworkResultDTO convertSearchHit(SearchHit hit) {
        try {
            ...
            return SearchResponseDTO.HomeworkResultDTO.builder()
                    ...
                    .author(SearchResponseDTO.AuthorDTO.builder()
    ...
    private Map<String, List<SearchResponseDTO.FacetEntryDTO>> extractFacets(SearchResponse response,
                                                                             SearchRequestDTO request) {
        ...
        try {
            ...
            if (response.getAggregations() != null) {
                ...
                if (tagsAgg != null) {
                    List<SearchResponseDTO.FacetEntryDTO> tagsList = tagsAgg.getBuckets().stream()
                            .map(bucket -> SearchResponseDTO.FacetEntryDTO.builder()
                ...
                if (authorsAgg != null) {
                    List<SearchResponseDTO.FacetEntryDTO> authorsList = authorsAgg.getBuckets().stream()
                            .map(bucket -> SearchResponseDTO.FacetEntryDTO.builder()
                                    .name(bucket.getKeyAsString())
                                    .count(bucket.getDocCount())
                                    .build())
